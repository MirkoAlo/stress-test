(function(){
	...
	var ready = function(fn) {
		if(!document.attachEvent){
			...
			document.readyState == "complete" ? fn() : document.attachEvent('onreadystatechange', function() {
	...
	var LazyElements = function(args){
		...
		this.sort = function(){
			this.elementsStart.sort(function(el1, el2){ return findPos(el1) - findPos(el2); });
		}
		...
		this.addAppearCallback = function(cb){
			this.callbacks.push(cb);
		}
		
		this.setMininterval = function(min){
			this.min = min;
		}
		...
		this.checkAppear = function(){
			...
			if(this.last && (now-this.last)<this.min){
				...
				if(this.timeout===undefined){
					...
					this.timeout = setTimeout(function(){delete me.timeout; me.checkAppear()}, this.last+this.min-now);
				}
				return;
			}
			delete this.timeout;
		...
		this.checkOnScroll = function(interval){
			var me = this;
			onScroll(function(){
				me.checkAppear(); 
			}, interval);
